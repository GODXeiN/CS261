...
def create_project():
    ...
    if request.method=='POST':
        ...
        if len(title)>0:
            ...
            project=Project(managerID=managerid, dateCreated=today_unix, title=title, dateLastSurveyed=today_unix,dateLastRiskCalculation=today_unix,updateInterval=7)
    ...
    if request.method=='POST':
        ...
        if new_budget and not new_deadline and not cost_to_date:
            ...
            if budget<cost:
                ...
                flash("Warning! Your project's current cost exceeds the budget by " + str(round(excess,4)) + "%.", category='warning')
